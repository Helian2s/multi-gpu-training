...
| AWS API and ECR | AWS CLI 2.33.27 is authenticated with profile `finetuning-local` as account `037678282394` in `us-west-2`; private ECR repositories `multi-gpu-training-pytorch` and `multi-gpu-training-nemo` exist with immutable tags, AES256 encryption, scan-on-push, and untagged-image cleanup after 7 days; after approved ECR image cleanup on 2026-07-16, both repositories have `imageCount=0` and `totalCompressedBytes=0`; temporary Docker configs were used for known-good ECR pushes; ECR pull-only instance-role simulation previously passed and denied `ecr:PutImage`; `FinetuningGpuS3Access` default version `v8` permits pinned input reads plus approved AWS artifact prefixes through EXP-12 and `QUAL-A2`; AMI is `ami-04b4c34375925db5f`, security group is `sg-0797f3b8520d4efa9`, IMDSv2 is required, and `session-manager-plugin` 1.2.835.0 is installed; read-only checks on 2026-07-16 found no active/stopped EC2 instances, no EBS volumes, no snapshots, no Elastic IPs, no NAT gateways, and no load balancers in `us-west-2`; the project S3 bucket still contains about 72.0 GB across 3460 objects | AWS EXP-01 through EXP-09 raw measurements are staged to S3; EXP-12 completed and is reported; AWS container images must be rebuilt and republished to ECR before any future AWS launch; future AWS retries must recreate cache volumes |
...
1. Create a GitHub Actions OIDC publishing role before automated ECR image
   pushes.
2. Before any future AWS launch, rebuild and republish the required PyTorch or
   NeMo image to ECR, then record the new immutable digest and scan disposition.
