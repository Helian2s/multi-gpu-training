...
RUN_DIR="${RUN_DIR:-/workspace/multi-gpu-training/artifacts/runs/EXP-10/${RUN_ID}}"
RAW_DIR="${RUN_DIR}/raw"
METRICS_DIR="${RUN_DIR}/metrics"
EXPECTED_DEVICES="${EXPECTED_DEVICES:-2}"

mkdir -p "${RAW_DIR}" "${METRICS_DIR}"

exec > >(tee "${RAW_DIR}/collect_exp10.log") 2>&1

echo "EXP-10 collection started at $(date -u +%Y-%m-%dT%H:%M:%SZ)"
echo "RUN_ID=${RUN_ID}"
echo "RUN_DIR=${RUN_DIR}"
echo "EXPECTED_DEVICES=${EXPECTED_DEVICES}"
...
nvidia-smi nvlink --status 2>&1 | tee "${RAW_DIR}/nvidia-smi-nvlink-status.txt" || true
...
expected = int(os.environ.get("EXPECTED_DEVICES", "2"))
payload = {
...
if payload["device_count"] != expected:
    raise SystemExit(f"expected exactly {expected} CUDA devices, got {payload['device_count']}")
PY
...
echo "EXP-10 collection finished at $(date -u +%Y-%m-%dT%H:%M:%SZ)"
